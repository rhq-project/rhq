...
2) Add the following external resources (where RHQ is the root directory of the RHQ source code): 
   ...
   -------------------------------------------------------------
    ...
    http://java.sun.com/jsp/jstl/functions   RHQ/etc/tlds/fn.tld
...
Note, you do not need to configure external resources for the JSF Core (f), JSF HTML (h), and JSF Facelets (ui) namespaces, since IDEA provides built-in mappings for those. However, I stuck the tld files for these three namespaces in RHQ/etc/ in case they're of use to Eclipse users.
