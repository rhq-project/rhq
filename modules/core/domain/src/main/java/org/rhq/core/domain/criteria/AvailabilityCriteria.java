...
public class AvailabilityCriteria extends Criteria {
    ...
    private Long filterStartTime; // note, this is actually used to perform an interval check, see below for more  
     ...
     *  
     * @param filterStartTime NOT NULL, in *SECONDS* (not ms)
     * @param filterEndTime   NOT NULL,  >= filterStartTime, in *SECONDS* (not ms)
     */
    public void addFilterInterval(Long filterStartTime, Long filterEndTime) {
        ...
        String filterFragment = "" //
            ...
            + " AND (  ( availability.startTime >= ? AND availability.startTime <= " + filterEndTime + " )" // interval straddles :start
