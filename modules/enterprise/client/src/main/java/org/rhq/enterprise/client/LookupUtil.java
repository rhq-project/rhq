...
public final class LookupUtil {
    ...
    public static ResourceManagerRemote getResourceManagerRemote() {
        return lookupRemote(ResourceManagerBean.class);
    }

    public static DataAccessRemote getDataAccess() {
        return lookupRemote(DataAccessBean.class);
    }
    ...
    public static SubjectManagerRemote getSubjectManagerRemote() {
    ...
    private static <T> String getLocalJNDIName(
     ...
     */
    private static <T> String getRemoteJNDIName(
    Class<? super T> beanClass) {
        return (embeddedDeployment ? "" : (RHQConstants.EAR_NAME + "/")) + beanClass.getSimpleName() + "/remote";
    }

    @SuppressWarnings("unchecked")
    private static <T> T lookupLocal(Class<T> type) {
        try {
    ...
    private static <T> T lookupRemote(Class<? super T> type) {
        try {
            ...
            throw new RuntimeException("Failed to lookup remote interface to EJB " + type + " at " + getRemoteJNDIName(type), e);
        }
    }
     ...
     *
     * @throws javax.naming.NamingException when resource not found
     */
