 ...
 */
public class FloatSetupValidityChecker implements SetupValidityChecker {
    private Float m_min;
    private Float m_max;

    /**
     * Defines the checker that validates the new value as a float that is between <code>min_value_allowed</code> and
     ...
     */
    public FloatSetupValidityChecker(Float min_value_allowed, Float max_value_allowed) {
        m_min = min_value_allowed;
        m_max = max_value_allowed;
    }

    /**
     * Checks to make sure the <code>value_to_check</code> is a valid float within the defined range.
     *
    ...
    public boolean checkValidity(String pref_name, String value_to_check, Preferences preferences, PrintWriter out) {
        try {
            float new_float = Float.parseFloat(value_to_check);

            if (m_min != null) {
                if (new_float < m_min.floatValue()) {
            ...
            out.println(CommI18NFactory.getMsg().getMsg(CommI18NResourceKeys.SETUP_NOT_A_FLOAT, value_to_check));
            return false;
        }
