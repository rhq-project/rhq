 ...
 */
public class IntegerSetupValidityChecker implements SetupValidityChecker {
    private Integer m_min;
    private Integer m_max;

    /**
     * Defines the checker that validates the new value as an integer that is between <code>min_value_allowed</code> and
    ...
    /**
     * Checks to make sure the <code>value_to_check</code> is a valid integer within the defined range.
     *
    ...
    public boolean checkValidity(String pref_name, String value_to_check, Preferences preferences, PrintWriter out) {
        try {
            ...
            out.println(CommI18NFactory.getMsg().getMsg(CommI18NResourceKeys.SETUP_NOT_AN_INTEGER, value_to_check));
            return false;
        }
