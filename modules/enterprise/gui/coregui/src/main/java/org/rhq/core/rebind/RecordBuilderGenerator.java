...
public class RecordBuilderGenerator extends Generator {
    ...
    private SourceWriter getSourceWriter(TreeLogger logger, GeneratorContext ctx, String packageName, String className,
    ...
    private boolean init(TreeLogger logger, GeneratorContext context, String typeName) throws UnableToCompleteException {
        ...
        try {
            ...
            logger.log(TreeLogger.ERROR, "Could not find type '" + typeName
                + "'; please see the log, as this usually indicates a previous error ", e);
        ...
        sourceWriter = getSourceWriter(logger, context, packageName, simpleStubClassName, requestedClass
    ...
    private void writeBuildRecordMethod(JField[] fields, SourceWriter sw) {
        sw.println();
        sw.println("protected final com.smartgwt.client.data.Record buildRecord("
            + requestedClass.getQualifiedSourceName() + " entity) throws Throwable {");
        sw.indent();
