    ...
    public static void storeDashboard(final Dashboard dashboard) {
        ...
        dashboardService.storeDashboard(dashboard, new AsyncCallback<Dashboard>() {
            public void onFailure(Throwable caught) {
                CoreGUI.getErrorHandler().handleError("Failed to store dashboard to server " + dashboard.getName() ,caught);
            }

            public void onSuccess(Dashboard result) {
                CoreGUI.getMessageCenter().notify(new Message("Saved dashboard " + result.getName(), Message.Severity.Info));
            }
        });
