...
public class ResourceChannelsUIBean extends PagedDataTableUIBean {
    public static final String MANAGED_BEAN_NAME = "ResourceChannelsUIBean";

    public ResourceChannelsUIBean() {
    }

    public String deleteSelectedResourceChannels() {
        ...
        if ((channelIds != null) && (channelIds.length > 0)) {
            try {
                ...
                manager.unsubscribeResourceFromChannels(subject, resourceId, channelIds);

                FacesContextUtility.addMessage(FacesMessage.SEVERITY_INFO, "Unsubscribed [" + channelIds.length
                    + "] channels from resource");
            } catch (Exception e) {
                FacesContextUtility.addMessage(FacesMessage.SEVERITY_ERROR,
                    "Failed to unsubscribe one or more channels from resource", e);
            }
        }
    ...
    public DataModel getDataModel() {
        if (dataModel == null) {
            dataModel = new ResourceChannelsDataModel(PageControlView.ResourceChannelsList, MANAGED_BEAN_NAME);
        }
    ...
    private class ResourceChannelsDataModel extends PagedListDataModel<ChannelComposite> {
        ...
        public PageList<ChannelComposite> fetchPage(PageControl pc) {
            ...
            PageList<ChannelComposite> results = manager.findResourceSubscriptions(subject, id, pc);
            return results;
        }
    }

    private String[] getSelectedResourceChannels() {
        return FacesContextUtility.getRequest().getParameterValues("selectedResourceChannels");
    }
