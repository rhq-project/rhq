...
public class WsAlertDefinitionManagerTest extends AssertJUnit implements TestPropertiesInterface {
    ...
    void testGetAlertDefinitionById() {
        ...
        assertNotNull("Expected to get back an alert " + "definition for id " + alertDef.id, WEBSERVICE_REMOTE
            .getAlertDefinition(subject, alertDef.id));
    }
    ...
    @Test(enabled = TESTS_ENABLED)
    void testFindAlertsWithFiltering() throws InterruptedException, MalformedURLException, SecurityException,
        IllegalArgumentException, ClassNotFoundException, NoSuchMethodException, InstantiationException,
        IllegalAccessException, InvocationTargetException, LoginException_Exception, FileNotFoundException {

        //attempt proxy stufff
        URL gUrl = WsUtility.generateRemoteWebserviceURL(WebservicesManagerBeanService.class, host, port, useSSL);
        BindingProvider bp = (BindingProvider) WEBSERVICE_REMOTE;
        bp.getRequestContext().put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY, gUrl.toString());
        ...
        assertEquals("Expected to get back one alert for alert definition '" + alertDef1Name + "'", 1, alerts.size());
    }
    ...
    ResourceOperationSchedule fireEvents(Resource resource, String severity, int numberOfEvents) {
        ...
        WsConfiguration parameters = createWsConfigurationParameters(resource, severity, numberOfEvents, details);
    ...
    Configuration createParameters(Resource resource, String severity, int numberOfEvents, String details) {
        ...
        params.propertyListOrPropertySimpleOrPropertyMap = params.getPropertyListOrPropertySimpleOrPropertyMap();
    ...
    public static WsConfiguration createWsConfigurationParameters(Resource resource, String severity,
