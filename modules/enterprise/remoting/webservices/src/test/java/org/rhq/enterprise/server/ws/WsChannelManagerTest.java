 ...
 */
@Test(groups = "ws")
public class WsChannelManagerTest extends AssertJUnit implements
	...
	void testCRUD() throws ChannelException_Exception {
		...
		List<Channel> channels = WEBSERVICE_REMOTE.findChannels(subject,
				pc_unlimited);
		...
		Channel testChannel = WEBSERVICE_REMOTE.createChannel(subject,
				newChannel);
		...
		testChannel = WEBSERVICE_REMOTE.getChannel(subject, channels.get(0)
				.getId());
		assertNotNull("test channel should exist.", testChannel);
		assertEquals("test-channel-0", testChannel.getName());
		...
		testChannel = WEBSERVICE_REMOTE
				.getChannel(subject, testChannel.getId());
		assertNotNull("test channel should exist.", testChannel);
		assertEquals("test-channel-0", testChannel.getName());
		assertEquals("description-1", testChannel.getDescription());
	...
	void testFindByCriteria() throws ChannelException_Exception {
		...
		ChannelCriteria criteria = new ChannelCriteria();
		channels = WEBSERVICE_REMOTE.findChannelsByCriteria(subject, criteria);
		assertEquals("empty criteria failed.", channels.size(),
				numRealChannels + 3);
	...
	void testDeploy() throws InterruptedException, IOException,
			ResourceTypeNotFoundException_Exception, ChannelException_Exception {
		...
		List<Resource> tomcatServers = WEBSERVICE_REMOTE
				.findResourcesByCriteria(subject, criteria);

		assertNotNull(
				"Test requires and available Tomcat Server in inventory.",
				tomcatServers);
		...
		List<Resource> wars = WEBSERVICE_REMOTE.findResourcesByCriteria(
				subject, criteria);
		Resource war = null;
		...
		if ((null != wars) && !wars.isEmpty()) {
			...
			for (int i = 0; (i < 60); ++i) {
				...
				wars = WEBSERVICE_REMOTE.findResourcesByCriteria(subject,
						criteria);
				if ((null == wars) || wars.isEmpty()) {
					break;
				}
			...
			assertTrue("test-channel-war should not exist",
					((null == wars) || wars.isEmpty()));
			System.out
		...
		criteria.setFilterName("Tomcat VHost (localhost)");
		criteria.setFilterCurrentAvailability(AvailabilityType.UP);
		List<Resource> vhosts = WEBSERVICE_REMOTE.findResourcesByCriteria(
				subject, criteria);
		...
		ResourceType warType = WEBSERVICE_REMOTE
				.getResourceTypeByNameAndPlugin(subject,
						"Tomcat Web Application (WAR)", "Tomcat");
		assertNotNull("Test requires Tomcat WAR resource type.", warType);

		// read in the file
		File file = new java.io.File(
				"./src/test/resources/test-channel-war.war");
		if ((file == null) || (!file.exists())) {
			file = new java.io.File(
					"../scripts/src/test/resources/test-channel-war.war");
		}
		if ((file == null) || (!file.exists())) {
		...
		inputStream.close();
		assertTrue("Could not completely read file " + file.getName(),
				(offset == fileBytes.length));

		// get package type for both create and update

		List<PackageType> packageTypes = WEBSERVICE_REMOTE.findPackageTypes(
				subject, "Tomcat Web Application (WAR)", "Tomcat");
		...
		ConfigurationDefinition deployConfigDef = WEBSERVICE_REMOTE
				.getPackageTypeConfigurationDefinition(subject, packageType
						.getId());
		assertNotNull("deployConfigDef should exist.", deployConfigDef);

		// explodeOnDeploy = deployConfigDef.getPropertyDefinitionSimple(
		// "explodeOnDeploy" );
		PropertyDefinitions propertyDefinitions = deployConfigDef
				.getPropertyDefinitions();
		assertNotNull("PropertyDefinitions have not been populated.",
				propertyDefinitions);
		PropertyDefinition explodeOnDeploy = (PropertyDefinition) locateProperty(
				propertyDefinitions, "explodeOnDeploy");
		...
		List<Property> prop = deployConfig
		...
		WEBSERVICE_REMOTE.createPackageBackedResource(subject, vhost.getId(),
				warType.getId(), null, null, file.getName(), "1.0", null,
		...
		InstalledPackage backingPackage = WEBSERVICE_REMOTE
				.getBackingPackageForResource(subject, war.getId());
		...
		List<Channel> channels = WEBSERVICE_REMOTE.findChannelsByCriteria(
				subject, criteriaChan);
		Channel channel;
		if (!channels.isEmpty()) {
		...
		assertTrue("channel should have existed or been created", (channel
				.getId() > 0));

		// test channel subscription
		// subscribedResources;
		List<Resource> subscribedResources = WEBSERVICE_REMOTE
				.findSubscribedResources(subject, channel.getId(), pc_unlimited);
		assertTrue(
				"test channel should not have resources",
				((null == subscribedResources) || subscribedResources.isEmpty()));
		...
		WEBSERVICE_REMOTE
				.subscribeResourceToChannels(subject, war.getId(), bag);

		subscribedResources = WEBSERVICE_REMOTE.findSubscribedResources(
				subject, channel.getId(), pc_unlimited);
		assertEquals("channel should have the test war", subscribedResources
				.size(), 1);

		// ChannelManager.unsubscribeResourceFromChannels( war.getId(),
		// [channel.getId()] );
		WEBSERVICE_REMOTE.unsubscribeResourceFromChannels(subject, war.getId(),
				bag);
		...
		List<PackageVersion> pvsInChannel = WEBSERVICE_REMOTE
				.findPackageVersionsInChannel(subject, channel.getId(), null,
						pc_unlimited);
		assertTrue("test channel should not have pvs",
				((null == pvsInChannel) || pvsInChannel.isEmpty()));

		List<Architecture> architectures = WEBSERVICE_REMOTE
				.findArchitectures(subject);
		...
		if ((file == null) || (!file.exists())) {
			file = new java.io.File(
					"../scripts/src/test/resources/test-channel-war-2.0.war");
		}
		if ((file == null) || (!file.exists())) {
			// THE FOLLOWING IS BAD, but there are some test situations where
			// mvn is NOT running from within project structure
		...
		PackageVersion pv = WEBSERVICE_REMOTE.createPackageVersion(subject,
				"test-channel-war-2.0.war", packageType.getId(), "2.0", null,
				fileBytes);
		...
		pvBag.add(pv.getId());
		WEBSERVICE_REMOTE.addPackageVersionsToChannel(subject, channel.getId(),
				pvBag);

		pvsInChannel = WEBSERVICE_REMOTE.findPackageVersionsInChannel(subject,
				channel.getId(), null, pc_unlimited);
		assertNotNull("pv should be in channel", pvsInChannel);
		assertEquals("unexpected pvs", pvsInChannel.size(), 1);
		assertEquals("unexpected pv returned", pvsInChannel.get(0).getId(), pv
				.getId());
		...
		InstalledPackage newBackingPackage = WEBSERVICE_REMOTE
				.getBackingPackageForResource(subject, war.getId());
		Assert.assertNotNull(newBackingPackage,
				"backing package should exist after update.");
	...
	private PropertyDefinition locateProperty(
			PropertyDefinitions propertyDefinitions, String name) {
