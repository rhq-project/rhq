 ...
 */
@Test(groups = "ws")
public class WsSubjectTest extends WsTestSetup {

	public WsSubjectTest() throws MalformedURLException, SecurityException,
	...
	public void testSubject() throws java.lang.Exception {
		...
		newUser = WEBSERVICE_REMOTE
				.login(testUserName, "updated-ws-test-password");
		...
		roleBag.add(roleId);
		WEBSERVICE_REMOTE.addRolesToSubject(user, Integer.valueOf(newUser.getId()),
				roleBag);

		// check that assignment occurred
		List<Role> attachedSubjects = WEBSERVICE_REMOTE.findSubjectAssignedRoles(
				user, newUser.getId(), pageControl);
		boolean found = false;
