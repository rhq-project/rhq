...
rem =============================================================================
rem RHQ Data Migration Script
rem
rem This file is used to migrate metric data from SQL server to the new RHQ storage server on a
rem Windows platform.
rem
rem This script is customizable by setting the following environment variables:
rem
rem    RHQ_DATA_MIGRATION_DEBUG - If this is defined, the script will emit debug
...
rem                                    is used.
rem
rem    RHQ_DATA_MIGRATION_JAVA_OPTS - Java VM command line options to be
...
rem                        RHQ_DATA_MIGRATION_ADDITIONAL_JAVA_OPTS instead.
rem
rem    RHQ_DATA_MIGRATION_ADDITIONAL_JAVA_OPTS - additional Java VM command
rem                                   line options to be passed into the VM.
rem                                   This is added to RHQ_DATA_MIGRATION_JAVA_OPTS; it
...
if defined RHQ_DATA_MIGRATION_DEBUG echo RHQ_SERVER_HOME: %RHQ_SERVER_HOME%
...
if defined RHQ_DATA_MIGRATION_DEBUG echo RHQ_SERVER_JBOSS_HOME: %RHQ_SERVER_JBOSS_HOME%
...
if not defined RHQ_SERVER_JAVA_EXE_FILE_PATH (
   if not defined RHQ_SERVER_JAVA_HOME (
      if defined RHQ_DATA_MIGRATION_DEBUG echo No JRE found - will try to use JAVA_HOME: %JAVA_HOME%
...
if defined RHQ_DATA_MIGRATION_DEBUG echo RHQ_SERVER_JAVA_HOME: %RHQ_SERVER_JAVA_HOME%
if defined RHQ_DATA_MIGRATION_DEBUG echo RHQ_SERVER_JAVA_EXE_FILE_PATH: %RHQ_SERVER_JAVA_EXE_FILE_PATH%
...
if not defined RHQ_DATA_MIGRATION_JAVA_OPTS (
   set RHQ_DATA_MIGRATION_JAVA_OPTS=-Xms512M -Xmx512M -XX:PermSize=128M -XX:MaxPermSize=128M -Djava.net.preferIPv4Stack=true -Dorg.jboss.resolver.warning=true
)

rem Add the JVM opts that we always want to specify, whether or not the user set RHQ_CCM_JAVA_OPTS.
if defined RHQ_DATA_MIGRATION_DEBUG (
...
set RHQ_DATA_MIGRATION_JAVA_OPTS=%RHQ_DATA_MIGRATION_JAVA_OPTS% -Djava.awt.headless=true -Drhq.server.properties-file=%RHQ_SERVER_HOME%\bin\rhq-server.properties -Drhq.server.basedir=%RHQ_SERVER_HOME%

rem Sample JPDA settings for remote socket debugging
rem set RHQ_DATA_MIGRATION_JAVA_OPTS=%RHQ_DATA_MIGRATION_JAVA_OPTS% -Xrunjdwp:transport=dt_socket,address=8787,server=y,suspend=y

if defined RHQ_DATA_MIGRATION_DEBUG echo "RHQ_DATA_MIGRATION_JAVA_OPTS: %RHQ_DATA_MIGRATION_JAVA_OPTS%"
if defined RHQ_DATA_MIGRATION_DEBUG echo "RHQ_DATA_MIGRATION_ADDITIONAL_JAVA_OPTS: %RHQ_DATA_MIGRATION_ADDITIONAL_JAVA_OPTS%"
...
if defined RHQ_DATA_MIGRATION_DEBUG echo _JBOSS_MODULEPATH: %_JBOSS_MODULEPATH%


echo "Starting RHQ Data Migration ..."

rem start the AS instance with our main data migration module
"%RHQ_SERVER_JAVA_EXE_FILE_PATH%" %RHQ_DATA_MIGRATION_JAVA_OPTS% %RHQ_DATA_MIGRATION_ADDITIONAL_JAVA_OPTS% -jar "%RHQ_SERVER_JBOSS_HOME%\jboss-modules.jar" -mp "%_JBOSS_MODULEPATH%" org.rhq.rhq-data-migration %*
