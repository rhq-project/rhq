 ...
 */
class AlertNotificationLogPurge extends PurgeTemplate<Integer> {
    ...
    private static final String QUERY_SELECT_KEYS_FOR_PURGE_POSTGRES = "" //
        + "SELECT nl.ID FROM RHQ_ALERT_NOTIF_LOG nl, RHQ_ALERT a " //
        + "WHERE nl.ALERT_ID = a.ID and a.CTIME BETWEEN ? AND ? LIMIT ?";

    private static final String QUERY_SELECT_KEYS_FOR_PURGE_ORACLE = "" //
        + "SELECT nl.ID FROM RHQ_ALERT_NOTIF_LOG nl, RHQ_ALERT a " //
        + "WHERE nl.ALERT_ID = a.ID and a.CTIME BETWEEN ? AND ? AND ROWNUM <= ?";
