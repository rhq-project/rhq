 ...
 */
class CallTimeDataValuePurge extends PurgeTemplate<Integer> {
    private static final String ENTITY_NAME = "CallTimeDataValue";

    private static final String QUERY_SELECT_KEYS_FOR_PURGE_POSTGRES = "" //
        + "SELECT ID FROM RHQ_CALLTIME_DATA_VALUE WHERE END_TIME < ? LIMIT ?";

    private static final String QUERY_SELECT_KEYS_FOR_PURGE_ORACLE = "" //
        + "SELECT ID FROM RHQ_CALLTIME_DATA_VALUE WHERE END_TIME < ? AND ROWNUM <= ?";

    private static final String QUERY_PURGE_BY_KEY = "DELETE FROM RHQ_CALLTIME_DATA_VALUE WHERE ID = ?";

    private final long deleteUpToTime;

    CallTimeDataValuePurge(DataSource dataSource, UserTransaction userTransaction, long deleteUpToTime) {
