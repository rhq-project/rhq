 ...
 */
public class PartitionEventPurge extends PurgeTemplate<Integer> {
    private static final String ENTITY_NAME = "PartitionEvent";

    private static final String QUERY_SELECT_KEYS_FOR_PURGE = "" //
        + " SELECT " //
        + "   e.ID " //
        + " FROM RHQ_PARTITION_EVENT e " //
        + " WHERE " //
        + "   NOT EXISTS(SELECT " //
        + "                1 " //
    ...
    private static final String QUERY_PURGE_BY_KEY = "DELETE FROM RHQ_PARTITION_EVENT WHERE ID = ?";

    private final long deleteUpToTime;

    PartitionEventPurge(DataSource dataSource, UserTransaction userTransaction, long deleteUpToTime) {
