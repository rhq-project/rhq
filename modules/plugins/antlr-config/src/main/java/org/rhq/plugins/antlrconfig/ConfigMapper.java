...
//            }
    ...
    private void createProperty(Tree tree, Configuration configuration, PropertyList parentList, PropertyMap parentMap) {
        PropertyDefinition propDef = pathConvertor.getPropertyDefinition(TreePath.getPath(tree, treeTypeNames));
        ...
        if (prop instanceof PropertySimple) {
            ((PropertySimple) prop).setValue(tree.getText());
        } else if (prop instanceof PropertyList) {
            parentList = (PropertyList) prop;
            parentMap = null;
        } else if (prop instanceof PropertyMap) {
            parentMap = (PropertyMap) prop;
            parentList = null;
        }
        
        for(int i = 0; i < tree.getChildCount(); ++i) {
            createProperty(tree.getChild(i), configuration, parentList, parentMap);
        }
    }
