      ...
      implements ResourceComponent, MeasurementFacet, OperationFacet
{
         ...
         throws Exception
   {
      ...
      if (!snmpSession.ping())
      {
         throw new InvalidPluginConfigurationException("Failed to connect to SNMP agent at " + snmpSession + ". Make sure 1) the managed Apache server has been instrumented with the JON SNMP module, 2) the Apache server is running, and 3) the SNMP agent host, port, and community are set correctly in this resource's connection properties.");
      }
      ...
      if (url != null)
      {
         ...
         {
            throw new InvalidPluginConfigurationException("Value of '" + PLUGIN_CONFIG_PROP_URL + "' connection property ('" + url + "') is not a valid URL.");
         }
      }
   ...
   public void getValues(MeasurementReport report, Set<MeasurementScheduleRequest> schedules) throws Exception
   {
      ...
      if (!snmpSession.ping())
      {
         throw new Exception("Failed to connect to SNMP agent at " + snmpSession + " - aborting metric collection...");
      }
   ...
   @NotNull
   public static SNMPSession getSNMPSession(SNMPClient snmpClient, Configuration pluginConfig)
   ...
   /**
    * Return the absolute path of this Apache server's control script (e.g. "C:\Program Files\Apache Group\Apache2\bin\Apache.exe").
    *
    * @return the absolute path of this Apache server's control script (e.g. "C:\Program Files\Apache Group\Apache2\bin\Apache.exe")
    */
   @NotNull
   ...
   static void addSnmpMetricValueToReport(MeasurementReport report, MeasurementScheduleRequest schedule, SNMPValue snmpValue, boolean valueIsTimestamp)
