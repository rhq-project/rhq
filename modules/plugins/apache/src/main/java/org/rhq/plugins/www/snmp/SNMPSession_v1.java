...
class SNMPSession_v1 implements SNMPSession
{
   ...
   public boolean ping()
   {
      ...
      {
         log.debug("Error while pinging SNMP " + this.version + " agent at " + this + ". SNMP GETNEXT request for iso(1) failed - " + e);
         return false;
      }
      boolean errorOccurred = (response.getErrorStatus() != PDU.noError);
      if (errorOccurred)
      {
         log.error("Error while pinging SNMP " + this.version + " agent at " + this + ". SNMP GETNEXT request for iso(1) failed - " + response.getErrorStatusText());
      }
   ...
   protected PDU createPDU(String mibName, int type)
   ...
   private SNMPValue getValue(String mibName, int pduType)
         throws SNMPException
   {
   ...
   @NotNull
   private PDU sendRequest(PDU request, String mibName)
         throws SNMPException
   {
