...
public class ApachePluginTest
{
   ...
   public void testMeasurementComponent()
   throws Exception
   {
      Resource platform = PluginContainer.getInstance().getInventoryManager().getPlatform();
      for (Resource server : platform.getChildResources())
      {
         ...
         for (Resource service : services)
         {
            ...
            if (serviceComponent instanceof MeasurementFacet)
            {
               ...
               metricList.add(new MeasurementScheduleRequest(1, "wwwSummaryInRequests", 1000, true, DataType.MEASUREMENT));
               metricList.add(new MeasurementScheduleRequest(2, "wwwRequestInRequests.GET", 1000, true, DataType.MEASUREMENT));
