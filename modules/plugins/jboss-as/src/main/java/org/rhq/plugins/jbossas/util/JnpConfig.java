...
/**
 * This class will parse the passed File in the getConfig method. This file is normally
 * the jboss-service.xml file in the conf directory of the JBoss Server.
 *
 * Using a SaxParser it will look for the MBean tag for Naming, when it finds that it is
 * in that tag, it will then look for the Port attribute to determine the jnp port
 *
 * The JBossServerHandler class handles the searching, by determining when the SAX parser
 * has started reading the element that has the text that is being searched, and when that element
 * has ended.
 *
