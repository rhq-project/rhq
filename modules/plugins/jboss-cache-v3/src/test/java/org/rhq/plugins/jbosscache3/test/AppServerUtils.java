...
public class AppServerUtils {
	...
	/**
	 * @return The application server resource from the plugin container's
	 *         inventory.
	 * 
	 ...
	 * <p>
	 * The proxy is created using the AS component classloader and therefore
	 * cannot be directly cast to the facetInterface, which is most probably
	 * loaded in a different classloader.
	 * <p>
	 * The return type is therefore an object and you have to use reflection to
	 * work with it.
	 * 
	 * @param <T>
	 * @param facetInterface
	 *            the proxy interface class to hide the application server
	 *            component behind
	 * @return the proxy
	 * @throws Exception
	 */
	public static <T> Object getASComponentProxy(Class<T> facetInterface)
			throws Exception {
		...
		try {
			...
			ClassLoader cl = PluginContainer.getInstance()
					.getPluginComponentFactory().getResourceClassloader(
							asResource);
			...
			return ComponentUtil.getComponent(asResource.getId(),
					resourceSpecificFacetInterface, FacetLockType.WRITE,
	...
	public static <T> T getRemoteObject(String jndiName, Class<T> clazz)
			throws Exception {
		InitialContext initialContext = getAppServerInitialContext();
		return clazz.cast(initialContext.lookup(jndiName));
	}

	public static InitialContext getAppServerInitialContext()
			throws NamingException {
		ResourceContainer resourceContainer = PluginContainer.getInstance()
				.getInventoryManager().getResourceContainer(getASResource());

		Configuration asConfiguration = resourceContainer.getResourceContext()
				.getPluginConfiguration();

		Properties env = new Properties();
		env.setProperty(Context.PROVIDER_URL, asConfiguration.getSimpleValue(
				"namingURL", null));
	 ...
	 * @param instance
	 *            the instance to invoke the method upon
	 * @param methodArgTypesAndValues
	 *            the method argument types and values
	 * @return the result of the method call
	 * @throws Exception
	 *             on error
	 */
	public static Object invokeMethod(String methodName, Object instance,
			MethodArgDef... methodArgTypesAndValues) throws Exception {
		Class<?>[] argTypes = null;
	...
	private static DeploymentManager getDeploymentManager() throws Exception {
		...
		Object connection = invokeMethod("getConnection", asComponent,
				(MethodArgDef[]) null);
		// I wonder why this cast works... where does the DeploymentManager get
		// loaded from in plugin and in here?
		return (DeploymentManager) invokeMethod("getDeploymentManager",
				connection, (MethodArgDef[]) null);
	}
}
