...
public class JndiResourceDiscoveryComponent
   implements ResourceDiscoveryComponent<JBossServerComponent>
{
   /**
   ...
   public Set<DiscoveredResourceDetails> discoverResources(ResourceDiscoveryContext<JBossServerComponent> context)
   {
      ...
      Map<String, DiscoveredResourceDetails> cacheMap = ResourceCache.getResourceMap(context.getResourceType(), context.getParentResourceComponent());
      ...
      {
         discoveredResources.add(new DiscoveredResourceDetails(r.getResourceType(),
            r.getResourceKey(),
            r.getResourceName(),
            r.getResourceVersion(),
            r.getResourceDescription(), null, null));
      }
   ...
   private void initializeFromScenario(ResourceDiscoveryContext<JBossServerComponent> context)
   {
      ...
      List<ScenarioResource> scenarioResources =
         scenarioLoader.getResources(context.getParentResourceComponent().getResourceContext().getResourceKey(), context.getResourceType().getName());
      Map<String, DiscoveredResourceDetails> scenarioResourceMap = new HashMap<String, DiscoveredResourceDetails>(scenarioResources.size());
      for (ScenarioResource scenarioResource : scenarioResources)
      {
         ...
         Map<String, MetricValueCalculator> valueCalculators = ScenarioLoader.createMetricValueCalculators(scenarioMetrics);
         ...
         Map<String, OperationHandler> operationHandlers = ScenarioLoader.createOperationHandlers(scenarioOperations);
         ...
         ScenarioResourceConfigurationError resourceConfigurationError =
               scenarioResource.getConfigurationError();
